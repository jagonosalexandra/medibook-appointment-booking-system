...
const Button = ({ label, variant, onClick, fullWidth = false, type = "button", disabled = false }) => {
    ...
    const variants = {
        ...
        disabled: "bg-primary/20 cursor-not-allowed"
    }
    ...
    return (
        <button
            type={type}
            className={`${selectedVariant} ${isFullWidth} px-6 py-2 mt-6 cursor-pointer rounded-xl font-semibold shadow-md transition-all duration-300`}
            onClick={onClick}
            disabled={disabled}
        >
            {label}
